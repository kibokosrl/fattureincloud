...
class Client
{
    ...
    public function createDoc($type, $doc)
    {
    ...
    {
        $request = new Struct\InfoListaRequest([
            'campi' => $campi,
        ]);
    ...
    protected function call($uri, Struct\Request $request, $method = 'POST')
    {
        ...
        try {
            ...
            $this->logger->info('SUCCESSFUL REQUEST ( ' . $uri . ' )  ', ['response' => $http_response->getBody()]);
        } catch (RequestException $e) {
            $this->logger->critical(
                'FATTUREINCLOUD REQUEST ERROR ( ' . $uri . ' ): ' . $e->getMessage(),
                [
                    'exception' => $e,
                ]
            );
